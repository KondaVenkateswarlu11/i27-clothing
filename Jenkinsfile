pipeline{
    ...
    environment{
        ...
        PACKAGE_VERSION = sh(script: 'npm version', returnStdout: true).trim()
        PACKAGE_NAME = sh(script: 'npm pack', returnStdout: true).trim()
    ...
    stages{
        ...
        stage("Unit Test") {
            ...
            steps {
                echo "This Stage will perform the unit tests for the ${env.APPLICATION_NAME} Node.js application"
                // Run the test script from package.json
                sh "npm run test"
            }
            post {
                always {
                    // Assuming Jest is configured to output test results in JUnit format
                    junit '**/junit.xml'
                    // If you have a different location for your test results, update the path accordingly
                }
            }
        }
/*
...
*/
        ...
        stage("Deploy to Docker-Dev"){
            ...
            steps{
                script{
                    ...
                    dockerDeploy('dev', '3003', '3000').call()
        ...
        stage("Deploy to Docker-test"){
            ...
            steps{
                script{
                    ...
                    dockerDeploy('tst', '3002', '3000').call()
                }
            }
        }
        ...
        stage("Deploy to Docker-stage"){
            ...
            steps{
                script{
                    imageValidation().call()
                    dockerDeploy('stage', '3001', '3000').call()
                }
            }
        }
...
def dockerBuildandPush(){
    return{
        ...
        sh "ls -al"
        // Copy the build output to the .cicd directory
        sh "cp -R ./build ./.cicd"               
        echo "*******************  Build Docker Image  ********************************"
        sh "docker build --force-rm --no-cache --pull --rm=true --build-arg JAR_SOURCE=i27-${env.APPLICATION_NAME}-.${env.PACKAGE_NAME}.${env.PACKAGE_VERSION} -t ${env.DOCKER_HUB}/${env.APPLICATION_NAME}:${GIT_COMMIT} ./.cicd"
...
// This method will be used to build the Node.js Application
def buildNodeApp(){
    return {
        echo "Building the ${env.APPLICATION_NAME} Node.js application"
